 ...
 */
static void prvInitialiseTCBVariables( tskTCB *pxTCB, const char * pcName, unsigned portBASE_TYPE uxPriority, const xMemoryRegion * const xRegions, unsigned short usStackDepth ) PRIVILEGED_FUNCTION;
...
portBASE_TYPE xReturn;
	...
	#if ( INCLUDE_xTaskGetIdleTaskHandle == 1 )
	{
		...
		xReturn = xTaskCreate( prvIdleTask, ( const char * ) "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), &xIdleTaskHandle );
	}
	#else
	{
		/* Create the idle task without storing its handle. */
		xReturn = xTaskCreate( prvIdleTask, ( const char * ) "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), NULL );
	}
	#endif	
...
tskTCB * pxTCB;
	...
	{
		uxCPULoadCount = 0;
		uxPercentLoadCPU = uxIdleTickCount;
		uxIdleTickCount = 0;
	} else uxCPULoadCount++;
