...
void irqDisable (const unsigned int irq)
{
	...
	if (irq <= 31) {
		*BCM2835_IRQ_DISABLE1 = mask;
		enabled[0] &= ~mask;
	}
	else if (irq <= 63) {
		*BCM2835_IRQ_DISABLE2 = mask;
		enabled[1] &= ~mask;
	}
	else if (irq < BCM2835_INTC_TOTAL_IRQ) {
		*BCM2835_IRQ_DISABLE_BASIC = mask;
		enabled[2] &= ~mask;
	}
}
