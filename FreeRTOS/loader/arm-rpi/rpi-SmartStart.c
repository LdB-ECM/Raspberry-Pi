...
. period in usec between triggers of the interrupt. If the weak irq stub
. has been overriden the function address can be set as 0 and it ignores.
						 ...
						 void (*ARMaddress)(void))					// Function to call on interrupt
{
		...
		8, 8, 4, 0))												// Get GPU clock (it varies between 200-450Mhz)
	{
		...
		Buffer[4] /= period_in_us;									// Divisor required
		if (Buffer[4] == 0) return 0;								// Invalid divisor so return with fail
		if (ARMaddress) OldHandler = setIrqFuncAddress(ARMaddress);	// Set new interrupt handler if non zero (with weak stub we can change vector outside)
		IRQ->EnableBasicIRQs.Enable_Timer_IRQ = true;				// Enable the timer interrupt IRQ
		ARMTIMER->Load = (uint32_t)((Buffer[4]-1) & 0xFFFFFFFF);	// Set the load value to divisor
